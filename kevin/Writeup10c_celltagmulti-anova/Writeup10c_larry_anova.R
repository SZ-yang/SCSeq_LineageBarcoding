...
covariates <- c("Cell.type.annotation", "Time.point", "Well", "assigned_lineage")
...
for(j in 1:p){
  if(p > 10 && j %% floor(p/10) == 0) {
    ...
    save(anova_mat, 
         file = paste0(out_folder, "Writeup10c_larry_anova_tmp.RData"))
  }
...
save(anova_mat, 
     file = paste0(out_folder, "Writeup10c_larry_anova.RData"))
