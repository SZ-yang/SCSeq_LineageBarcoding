...
def save_model_and_losses(trainer, loss_callback, config):
    save_name = os.path.join(config.save, 'scContrastiveLearn_adam_.ckpt')
...
def main():
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(os.getcwd(), "saved_models"),
        filename='scContrastiveLearn_adam_{epoch:02d}',
        save_last=True,
        save_top_k=2,
    ...
    # trainer
    trainer = pl.Trainer(
        callbacks=[GradientAccumulationScheduler({0: train_config.gradient_accumulation_steps}), checkpoint_callback, loss_callback, early_stopping_callback],
        gpus=torch.cuda.device_count(),
        max_epochs=train_config.epochs,
        check_val_every_n_epoch=1 if train_config.train_test else None,
        resume_from_checkpoint=train_config.checkpoint_path if train_config.load else None
    )
