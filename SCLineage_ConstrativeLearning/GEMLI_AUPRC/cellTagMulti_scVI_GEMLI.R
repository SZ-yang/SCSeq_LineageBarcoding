...
# Already computed in test_lineages_custom:
recall <- res[,"sensitivity"]
precision <- res[,"precision"]

# Drop undefined precision (when TP+FP == 0)
ok <- is.finite(recall) & is.finite(precision)
recall <- recall[ok]
precision <- precision[ok]

# PR convention: recall increasing
ord <- order(recall)
recall <- recall[ord]
precision <- precision[ord]

# (optional) enforce endpoints for cleaner integration
if (length(recall) > 0) {
  if (min(recall) > 0) {
    recall <- c(0, recall)
...
# AUPRC via trapezoid rule over recall
auprc <- sum(diff(recall) * (precision[-1] + precision[-length(precision)]) / 2, na.rm=TRUE)

png(paste0(plot_folder, "cellTagMulti_scVI_GEMLI_PR.png"),
    height = 1200, width = 1200, units = "px", res = 300)
...
# baseline precision = prevalence of positives among evaluated pairs
