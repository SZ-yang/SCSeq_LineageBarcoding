...
if train_config.train_test:
    ...
    train_batch_keys = random.sample(list(batch_all.keys()), int(train_config.train_test_ratio*num_batch))
    ...
    all_dataset = ds.SCDataset(batches=batch_all)
    train_loader = torch.utils.data.DataLoader(dataset=all_dataset, batch_size=train_config.batch_size, shuffle=False, num_workers=1) #num_workers=cpu_count()//2
