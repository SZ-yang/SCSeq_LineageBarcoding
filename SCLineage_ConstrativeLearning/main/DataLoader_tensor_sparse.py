...
5. Track the cell id for each batch (done)


left for improvements:

6. Documentation (actually batch size = self.batch_size*2)
7. it won't handle if a lineage only has one cell
            ...
            if len(self.avail_lineages) >= self.batch_size:
                ...
                for key in sampled_lineages_idx:
                    ...
                    if len(self.avail_lineages[key])>1: 
                        ...
                        # Modified to include cell IDs
                        self.lineage_info.append((key, sampled_2cell))
